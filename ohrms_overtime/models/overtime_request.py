...
class HrOverTime(models.Model):
    ...
    employee_id = fields.Many2one('hr.employee', default=lambda self: self.env["hr.employee"].search(
        ...
        # ('hr.employee', string='Employee',domain=_get_employee_domain, default=lambda self: self.env.user.employee_id.id, required=True)
    ...
    def submit_js(self):
        # user_ids = _get_uid_from_group(self, self.env.ref("a_role.group_station_staff").id)
        ...
        # )
        self.env['send.email.notification'].send(self.id, 'hr.overtime', 'Review', 'group', 'a_role.group_station_staff', self.name)
        return self.sudo().write({
            'state': 'submit'
        })
    ...
    def submit_so(self):
        ...
        #     subtype="mail.mt_comment",
        # )
        self.env['send.email.notification'].send(self.id, 'hr.overtime', 'Review', 'group', 'a_role.group_station_manager', self.name)
        return self.sudo().write({
            'state': 'review_so'
        })

    def submit_sm(self):
        # user_ids = _get_uid_from_group(self, self.env.ref("a_role.group_station_master").id)
        # partner_ids = [self.env["res.users"].browse(uid).partner_id.id for uid in user_ids]
        # self.message_post(
        #     body="[THIS IS AUTOMATIC MESSAGING SYSTEM. NO REPLY] \nDear User, This is outstanding transaction that need your action to submit.\nThanks",
        #     email_from="gaas@jakartamrt.co.id",
        ...
        #     subtype="mail.mt_comment",
        # )
        self.env['send.email.notification'].send(self.id, 'hr.overtime', 'Approve', 'group', 'a_role.group_station_master', self.name)
        return self.sudo().write({
            'state': 'review_sm'
        })
